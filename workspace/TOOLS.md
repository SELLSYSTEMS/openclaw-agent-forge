...
- Default model floor: `codex/gpt-5.4` with Codex CLI auth and the bundled Codex app-server harness as the intended runtime path
...
- General rule: before delivering artifacts through any system, run `<REPO_ROOT>/scripts/check-artifact-delivery.sh --target <target> <file>` or perform an equivalent live limit check for that destination.
