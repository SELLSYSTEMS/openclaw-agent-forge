...
```text
Decide whether the task belongs in OpenClaw, Node-RED, or a small local script. Prefer OpenClaw for agent coordination, Node-RED for durable automations and visual flows, and repo-local scripts for small explicit utilities. Avoid hidden cross-tool state and document the decision in memory/decisions.md.
```
