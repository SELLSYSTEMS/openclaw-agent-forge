...
- On 2026-09-13, native Codex recovered from transient model-access/stream errors and completed a long task, but the old projector retained a generic error and the reply selector suppressed the final after an earlier progress send. The outbox was empty because the final never reached transport. See [codex-telegram-final-delivery.md](codex-telegram-final-delivery.md) for the reproduced defects, terminal-status repair, strict recovery guards and real-runtime regression suite.
- Clean-install CI then found that the historical Telegram outbox base was present only in the live install, while Git contained only a guard on top of it. Test the official pristine package as well as idempotence on an existing host. Keep the base diff, provenance, exact hashes and executable regression tests in Git; do not hide an absent prerequisite by skipping validation.
...
- The first Codex app-server recovery guard was too narrow; v2 then recovered from visible text on `sessionLastAssistant`. That historical workaround was not a complete fix. The 2026-09-13 guard requires verified current-attempt terminal success, so previous/partial answers cannot hide quota failures or interruptions. Require the new terminal-status and delivery tests, not just v1/v2 markers.
...
- Do not edit a shell helper while an instance of that script is still executing. Bash can read changed bytes when a long-running child returns and fail with unrelated-looking syntax errors. Wait for completion or execute an immutable private snapshot before editing the source.
