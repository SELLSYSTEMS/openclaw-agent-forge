...
- Reproducibility: clean npm installation must reconstruct the historical Telegram text-outbox base before applying dependent guards. Track its diff, license and exact hashes; verify failure retention, receipt-based removal and no-write idempotence without accessing the live queue.
